...
// The second way of declaring an array
string[] names2 = new[] { "Kate", "Jack", "Rebecca", "Tom" };

// Looping through the names
for (int i = 0; i < names2.Length; i++)
{
    // output the item at index position
    WriteLine(names2[i]);
}
...
string[,] grid1 = new[,]
{
    ...
    {"Aardvak", "Bear", "Cat", "Dog" },
};
WriteLine(grid1[1,1]); // Ben
WriteLine($"Lower bound of the first dimension is: {grid1.GetLowerBound(0)}");
WriteLine($"Upper bound of the second dimension is: {grid1.GetUpperBound(0)}");
...
WriteLine($"Upper bound of array {jagged} is: {jagged[0].GetUpperBound(0)}");
